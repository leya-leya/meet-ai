...
2026-08-18 已按上述文件名完成七张 V1.0 最终页面截图。截图使用内部 UUID 文件名与脱敏验收文本，不包含真实转写、API Key、个人隐私或调试报错。
...
2026-08-18 使用一段 267.889 秒的 MP4 测试媒体完成真实 ASR 验证；`RealASRProvider` 在 110.58 秒内返回 995 个字符的非空转写文本。验证记录不保存媒体内容、完整转写或任何密钥。
...
2026-08-18 使用同一段 267.889 秒 MP4 在浏览器完成 V1.0 真实端到端验收：上传后由科大讯飞返回非空 transcript，DeepSeek 返回包含三个固定标题的摘要；随后完成标题、摘要与 transcript 编辑保存、标题搜索、重新打开、TXT/Markdown 导出和记录删除。导出接口均返回 HTTP 200，删除后详情接口返回 HTTP 404。真实媒体、完整转写和摘要原文均未写入 Git 或软著截图。
...
2026-08-18 已完成 V1.0 冻结验证：后端 `pytest -v` 共 72 项通过，前端 `npm run build` 通过，真实 ASR/LLM 用户流程与七张操作截图均完成，README 与前端版本统一为 `1.0.0`，最终提交标记为 `v1.0.0`。
